...
The repository also includes \texttt{generate\_internal\_datasets.py}, which
reproduces those two packaged internal CSVs exactly; this is an optional
provenance check and is not required for the main grading workflow.
...
\begin{enumerate}
    ...
    behavior.
    \item Optional reproducibility check: run
    \texttt{generate\_internal\_datasets.py} to regenerate
    \texttt{sample\_internal.csv} and \texttt{huge\_internal.csv}. This is a
    short helper script only; the intended TA path still uses the packaged zip.
\end{enumerate}
